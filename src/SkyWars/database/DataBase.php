...
class DataBase{
	 ...
	 */
	public function __construct(string $file, int $type = DataBase::DETECT, array $default = [], &$correct = null){
	 ...
	 * @phpstan-param array<string, mixed> $default
	 */
	public function load(string $file, int $type = DataBase::DETECT, array $default = []) : bool{
		$this->correct = true;
		$this->file = $file;

		$this->type = $type;
		if($this->type === DataBase::DETECT){
			$extension = explode(".", basename($this->file));
			$extension = strtolower(trim(array_pop($extension)));
			if(isset(DataBase::$formats[$extension])){
				$this->type = DataBase::$formats[$extension];
			}else{
				$this->correct = false;
			}
		}
	 ...
	 * @see json_encode
	 */
	public function setJsonOptions(int $options) : DataBase{
		if($this->type !== DataBase::JSON){
	 ...
	 * @see json_encode
	 */
	public function enableJsonOption(int $option) : DataBase{
		if($this->type !== DataBase::JSON){
	 ...
	 * @see json_encode
	 */
	public function disableJsonOption(int $option) : DataBase{
		if($this->type !== DataBase::JSON){
